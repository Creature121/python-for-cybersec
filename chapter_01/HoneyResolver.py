...
class HoneyResolver:
    def resolve(self, request, handler):
        ...
        if subdomain in subdomains:
            ...
            reply.add_answer(RR(rname=request.q.qname, rtype=QTYPE.A, rclass=1, ttl=300, rdata=A(ip)))
        else:
            reply = request.reply()
            reply.add_answer(RR(rname=request.q.qname, rtype=QTYPE.A, rclass=1, ttl=300, rdata=A(honey_ip)))
        return reply
